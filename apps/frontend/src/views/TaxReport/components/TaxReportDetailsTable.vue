...
const {
  ...
  computed(() => props.auditTrail),
  20,
);

// Typed cast: usePagination returns ComputedRef<unknown[]> to stay generic
const paginatedEvents = paginatedData as unknown as import('vue').ComputedRef<
  TaxLotHistoryEvent[]
>;
...
<template>
  ...
  >
    ...
    <div v-if="isLoading" class="space-y-2 p-6">
      <p
        class="mb-4 text-center text-[10px] font-black uppercase tracking-widest text-muted-foreground"
      >
          ...
          <TableHeader>
            <TableRow class="text-[10px] uppercase tracking-widest">
              ...
              <TableHead class="text-right">{{
                t('tax.audit.col_sale_price')
              }}</TableHead>
              <TableHead class="text-right">{{
                t('tax.audit.col_gain_loss')
              }}</TableHead>
              <TableHead class="text-right">{{
                t('tax.audit.col_fee')
              }}</TableHead>
            ...
            >
              ...
              <TableCell>
                <Badge
                  :class="[
                    ...
                    getOperationBadgeColor(event.operationType),
                  ]"
                >
              ...
              <TableCell>
                ...
                >
                  <Badge
                    ...
                    :style="
                      {
                        '--badge-hue': getDeterministicHue(event.exchangeName),
                      } as CSSVars
                    "
                  >
                    <CryptoIcon
                      :symbol="event.exchangeName"
                      :size="10"
                      colored
                    />
                    {{ event.exchangeName }}
              ...
              <TableCell class="text-right">
                ...
                <div
                  ...
                  class="text-[9px] font-mono text-muted-foreground/60 mt-0.5"
                  :title="`Converted on ${formatDateNumeric(new Date(event.fxRateDate))}`"
                >
                  fx {{ formatNumber(event.fxRate) }} ({{
                    formatDateNumeric(new Date(event.fxRateDate))
                  }})
                </div>
              </TableCell>
