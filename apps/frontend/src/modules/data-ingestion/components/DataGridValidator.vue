...
const handleRemap = (originalHeader: string, targetDomainProperty: unknown) => {
  ...
  wizard.previewTable.generatePreview(wizard.fileParser.rawRows.value, wizard.columnMapper.mapping.value);
};
...
const handleCellInput = (rowId: string, header: string, event: Event) => {
  ...
  if (targetDomainProperty) {
    if (targetDomainProperty === "metadata") {
      ...
      wizard.previewTable.updateRowField(rowId, targetDomainProperty as keyof TransactionRow["mappedData"], input.value);
    }
  }
};
</script>
...
<template>
  ...
  >
    <div class="flex flex-col h-full w-max min-w-full">
      <div
        class="bg-surface-2 border-b border-border flex items-center pr-2"
      >
      <div
        v-for="header in table.getFlatHeaders()"
        :key="header.id"
        class="flex-1 min-w-[150px] p-3 text-xs font-semibold text-muted uppercase tracking-wider border-r border-border last:border-r-0 flex flex-col gap-2"
      >
        ...
        >
          ...
          >
            <SelectValue :placeholder="t('ingestion.grid.mapping_unmapped')" />
          </SelectTrigger>
          <SelectContent>
    ...
    <div ref="parentRef" class="flex-1 overflow-y-auto overflow-x-hidden relative w-full">
      <div
      ...
      >
        ...
        >
          <div
            ...
            :key="cell.id"
            class="flex-1 min-w-[150px] border-r border-border-soft last:border-r-0 p-2 flex items-center"
          >
              ...
              <input
                ...
                :value="
                  cell.column.columnDef.meta.mappedTo === 'metadata'
                    ? rows[virtualRow.index].original.mappedData.metadata?.[cell.column.id] || ''
                    : rows[virtualRow.index].original.mappedData[
                        cell.column.columnDef.meta.mappedTo as keyof TransactionRow['mappedData']
                      ] as string
                "
                class="w-full bg-transparent px-2 py-1 outline-none text-sm num transition-colors rounded-sm"
                :class="[
                  (rows[virtualRow.index].original.errors as string[]).includes(
