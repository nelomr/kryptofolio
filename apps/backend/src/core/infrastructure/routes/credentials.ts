...
const credentialsApi = new Hono<Env>()
  ...
  .post(
    ...
    zValidator(
      ...
      z.object({
        payload: z.record(
          z.string().regex(/^[a-zA-Z0-9_+=/.-]{0,512}$/, 'INVALID_CREDENTIAL_FORMAT'),
        ),
      }),
    ),
    async (c) => {
      ...
      try {
        ...
        return c.json(
          {
            success: false,
            error: status === 401 ? 'VAULT_LOCKED' : 'VAULT_OPERATION_FAILED',
          },
          status,
        );
      }
    ...
    async (c) => {
      ...
      try {
        ...
        return c.json(
          {
            success: false,
            error: status === 401 ? 'VAULT_LOCKED' : 'FAILED_TO_TOGGLE_PROVIDER',
          },
          status,
        );
      }
