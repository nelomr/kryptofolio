...
/**
 * AesGcmCryptographyAdapter — Infrastructure adapter implementing ICryptographyPort.
 *
 * Uses AES-256-GCM for symmetric encryption. The master key is sourced from:
 * 1. Explicit password via key derivation (scrypt)
 * 2. OS Keyring via @napi-rs/keyring (preferred for local dev)
 * 3. KRYPTO_MASTER_KEY env variable (fallback / Docker)
 */
