...
export class SQLiteLedgerAdapter implements ILedgerPort, IFxRateLedgerPort {
  ...
  async getSpotTransactions(accountId?: string): Promise<LedgerSpotTransaction[]> {
    ...
    return rows.map(row => ({
      ...
      fee_asset_id: (row.fee_asset_id as string | null) ?? undefined,
      fee_amount: row.fee_amount ? toPreciseAmount(row.fee_amount as string) : undefined,
      total_fiat: row.total_fiat ? toPreciseAmount(row.total_fiat as string) : null,
      price_fiat: row.price_fiat ? toPreciseAmount(row.price_fiat as string) : null,
  ...
  async saveSpotTransaction(tx: LedgerSpotTransaction): Promise<void> {
    ...
    stmt.run(
      ...
      tx.total_fiat ? tx.total_fiat.toString() : null,
      tx.price_fiat ? tx.price_fiat.toString() : null,
      tx.fiat_currency,
      tx.timestamp,
      tx.status,
      tx.flag ?? null,
  ...
  async getFuturesTransactions(accountId?: string): Promise<LedgerFuturesTransaction[]> {
    ...
    return rows.map(row => ({
      ...
      settlement_asset_id: (row.settlement_asset_id as string | null) ?? undefined,
