...
- `source-format-profiles`: One small declarative unit per source, stating only what a header-name mapper structurally cannot — the fee's denomination, whether the reported amount already includes the fee, whether one row fills both directional sides, which columns are genuine transaction references rather than category labels, and an optional invariant the source's own data can fail. Selected from the header row by required-plus-forbidden signature matching that reports ambiguity instead of resolving it by declaration order, confirmable by the user, and carried to the backend so the wizard's preview and the persisted ledger cannot disagree. The reader stays source-agnostic and the user-confirmed column mapping is retained unchanged; the five unreachable exchange parsers, `REGISTERED_PARSERS` and `ICsvIngestionPort` are deleted. Scoped to group 14 phase 14βb, which runs before the fee phases that consume it; see D29–D33.
...
- `csv-data-ingestion`: Fiat magnitudes are sign-normalised and constrained; unknown `tx_type` values fail loudly instead of defaulting to `BUY`; transaction IDs are deterministic so re-ingestion is idempotent; the Kraken `wallet` column resolves sub-accounts. The submission additionally carries the selected source format profile, which is a required field on the ingestion contract rather than an optional one with a default.
...
- `domain-anti-corruption`: an unresolved value stops being coerced to `0` at the client boundary — the surviving twin of the fabricated-price defect, one layer out; frontend parsers are verified against real backend payloads instead of fixtures their own author wrote; and ingested transaction identifiers become deterministic rather than random, which the reconciliation model already assumed. Added after group 10 measured what the client does with a nullable payload; see D26–D28.
...
- `apps/frontend/src/core/infrastructure/csv/` — **deleted in full**, with `apps/frontend/src/core/domain/ports/ICsvIngestionPort.ts`. The five exchange parsers and `REGISTERED_PARSERS` are unreachable (the `MockTaxAdapter` the registry names as its consumer does not exist) and contradict the domain: `deposit → 'DEPOSIT'` where the classifier says `TRANSFER_IN`, `feeEur: 0` on every movement, and `Math.random()` in the identifier. Tasks 11.8 and 11.11 lose their Kraken/Bitvavo/BitUnix clauses to this deletion; the deterministic-identifier requirement moves to the wizard path, which is the live one.
- `packages/core-domain/src/domain/services/sourceProfile/` (new) — the profile table, its discriminated unions, header-signature detection, and the pure appliers both the wizard preview and backend ingestion call.
- `apps/frontend/src/modules/data-ingestion/` — a detected-profile surface with a user override inside step 1; `processAndSubmit` gains a fourth argument. The three-step flow, the `ParseResult` shape, and the user-confirmed column mapping are unchanged.
