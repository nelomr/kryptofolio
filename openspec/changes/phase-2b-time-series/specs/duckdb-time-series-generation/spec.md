...
2. Performing a second join against `exchange_rates` (SQLite) using the user's `base_currency` (read from `user_settings`) to convert the native price to the target reporting currency.
...
- **AND** the frontend Pinia Colada cache MUST be invalidated for this settings change
