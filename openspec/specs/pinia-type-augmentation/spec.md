...
- **WHEN** `inject(PORTFOLIO_PORT_KEY)` returns `undefined` because `app.provide()` was not called
- **THEN** the system SHALL throw an `Error` with a message containing the string `"PORTFOLIO_PORT_KEY"` and the word `"undefined"`

#### Scenario: Missing tax provider causes clear error

- **WHEN** `inject(TAX_PORT_KEY)` returns `undefined`
- **THEN** the system SHALL throw an `Error` with a message containing the string `"TAX_PORT_KEY"` and the word `"undefined"`
